# DockBridge Install Script

`install.sh` helps new users complete a local bootstrap flow:
...
- reuse an existing SSH-backed Docker context or create a new one
- if needed, collect a server name that becomes the default SSH alias and Docker context
- if needed, collect SSH target info
...
When Docker is available locally, the script first asks whether you want to:

- reuse an existing Docker context whose Docker host is `ssh://...`
- create a new SSH-backed Docker context

Only SSH-backed contexts are offered for reuse.
...
```bash
./install.sh --context-name dockbridge-remote --output /tmp/dockbridge-onboarding.log
```
